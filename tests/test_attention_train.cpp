// Attention AOT-backprop test (v1.3): reverse-mode must lower LAYER_ATTENTION
// to ATTENTION_GRAD_X/WQ/WK/WV/WO (recompute Q/K/V + per-head softmax) and
// Attention must be usable in a train() body. Task: "copy the previous token"
// — each position i must output the class of token i-1, which is impossible
// without cross-position attention. Loss must decrease and the model must
// classify correctly.
...
// Attention(d_model, heads) consumes a [N] token stream as ONE sequence of
// length N (B=1 v1.2 convention); the model must attend to the previous token.
static const char* ATT_NET = R"(
type Bs = Dynamic
network AttNet {
...
int main() {
    ...
    const std::string driver = "/tmp/ns_attn_driver.cpp";
    const std::string host   = "/tmp/ns_attn_host.cpp";
    const std::string bin    = "/tmp/ns_attn_host";
    {
...
int main() {
    ...
    // emb (8*16) + attn q/k/v/o (4*16*16) + fc (16*4) = 1216
    const size_t nw = 8 * 16 + 4 * 16 * 16 + 16 * 4;
    ...
    float lossT = loss0;
    const int epochs = 12000;
    for (int e = 0; e < epochs; e++)
        if (ns_runtime_train_step(m, xs, ys, N, &lossT, 0.01f) != 0)
    ...
    if (correct * 10 < total * 9) {  // need >= 90%
        ...
        return fail("attention classification not converged");
    }
    ...
    if (std::system(cmd.c_str()) != 0) {
        std::cerr << "FAIL: Attention train host link failed\n";
        return 1;
    }
    if (std::system(bin.c_str()) != 0) {
        std::cerr << "FAIL: Attention train host run failed\n";
        return 1;
    }

    std::cout << "PASS: Attention AOT backprop\n";
    return 0;
}